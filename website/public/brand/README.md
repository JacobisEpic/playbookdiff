...
| File                   | Used for                                                                                                                               |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `mascots-reading.png`  | The logo mark in the header and footer, the hero illustration, the favicon, the social preview card, and the repository README header. |
| `mascots-running.png`  | Spare illustration. Not currently placed on the site.                                                                                  |
| `wordmark.png`         | The word mark, in the site header and footer, and on the social preview card.                                                          |
...
The source crop is 364x297, which is the practical ceiling: the hero illustration is placed at 320 CSS pixels so it stays under native width, and anything much larger softens the outlines on a high-density display.
...
`../../app/opengraph-image.png` is the social preview card, regenerated by `../../scripts/generate-og-image.py` from the word mark, the mascots, and the site's own typefaces.
...
It appears on a `not received` row in the effective-configuration example, and on a severity label.
